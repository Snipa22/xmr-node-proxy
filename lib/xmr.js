...
function getJob(miner, activeBlockTemplate){
...
function processShare (miner, job, blockTemplate, nonce, resultHash){
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (hash.toString('hex') !== resultHash) {
            ...
            miner.messageSender('job', miner.getJob(miner, blockTemplate));
            return true;
        }
        miner.blocks += 1;
        process.send({
