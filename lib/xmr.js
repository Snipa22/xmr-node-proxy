...
function parse_algo_variant(algo_str, variant) {
    ...
    switch (algo_str) {
        ...
        case 'cn/half':
        case 'cryptonight/half':       return 9;

        case 'cn/gpu':
        case 'cryptonight/gpu':        return 11;

        case 'cn/wow':
        case 'cryptonight/wow':        return 12;

        case 'cn/r':
        case 'cryptonight/r':          return 13;

        case 'cn/rwz':
        case 'cryptonight/rwz':        return 14;

        case 'cn/zls':
        case 'cryptonight/zls':        return 15;

        case 'cn/double':
        case 'cryptonight/double':     return 16;
    }
    return 1;
}

function detectAlgo(default_pool_algo_set, block_version) {
    if ("cn/r" in default_pool_algo_set && "cn/2" in default_pool_algo_set) return block_version >= 10 ? "cn/r" : "cn/2"; // Monero fork
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
            ...
            let variant = parse_algo_variant(blockTemplate.algo, blockTemplate.variant);
            hash = foo(convertedBlob, variant, blockTemplate.height);
