...
function blobTypeRTM(blob_type_num) {
    return blob_type_num == 104;
}
...
function convertBlob(blobBuffer, blob_type_num){
    ...
    else if (blobTypeRTM(blob_type_num)) return cnUtil.convertRtmBlob(blobBuffer);
    else return cnUtil.convert_blob(blobBuffer, blob_type_num);
}
...
function constructNewBlob(blockTemplate, NonceBuffer, blob_type_num, ring){
    ...
    } else if (blobTypeRTM(blob_type_num)) {
        return cnUtil.constructNewRtmBlob(blockTemplate, NonceBuffer);
    } else {
...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cryptonote_zeph': return 13; // Zephyr
        case 'cryptonote_xla':  return 14; // XLA
        case 'cryptonote_dero': return 100; // Dero
        case 'raptoreum':       return 104; // RTM
    }
    return 0;
}
...
function hash_buff_diff(hash) {
    return baseDiff.div(bignum.fromBuffer(Buffer.from(hash.toByteArray().reverse())));
}
...
function processShare(miner, job, blockTemplate, params) {
    ...
    const hashDiff = hash_buff_diff(blobTypeGrin(blob_type) ? multiHashing.c29_cycle_hash(params.pow) : Buffer.from(params.result, 'hex'));
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (blobTypeGrin(blob_type)) {
            ...
            if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
                ...
                const shareBuffer = constructNewBlob(template, Buffer.from(nonce, 'hex'), blob_type);
