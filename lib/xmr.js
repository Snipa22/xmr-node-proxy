...
function blobTypeGrin(blob_type_num) {
    return blob_type_num == 8 || blob_type_num == 9;
}

function blobTypeDero(blob_type_num) {
    return blob_type_num == 10;
}
...
function constructNewBlob(blockTemplate, NonceBuffer, constructNewBlob, ring){
    if (blobTypeDero(blob_type_num)) {
        NonceBuffer.copy(blockTemplate, 39, 0, 4);
        return blockTemplate;
    } else return constructNewBlob(blockTemplate, NonceBuffer, blob_type_num, ring);
}
...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cryptonote_dero': return 10; // Dero
    }
    return 0;
}
