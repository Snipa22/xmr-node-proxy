...
function parse_blob_type(blob_type_str) {
    ...
    if (blob_type_str === 'forknote1') return 1;
    if (blob_type_str === 'forknote2') return 2;
    return 0;
}
...
function getJob(miner, activeBlockTemplate, bashCache) {
    ...
    if (typeof (activeBlockTemplate.coin) !== 'undefined') {
        miner.cachedJob.coin = activeBlockTemplate.coin;
    }
    return miner.cachedJob;
}
