...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cuckaroo':        return 8; // Swap/XTNC/MoneroV
    }
    return 0;
}
...
function getJob(miner, activeBlockTemplate, bashCache) {
    ...
    if (activeBlockTemplate.blob_type == 8) miner.cachedJob = {
        pre_pow:    blob,
        algo:       "cuckaroo",
        edgebits:   29,
        proofsize:  32,
	noncebytes: 4,
        ...
        height:     activeBlockTemplate.height,
        seed_hash:  activeBlockTemplate.seed_hash,
        target:     getTargetHex(miner.difficulty, activeBlockTemplate.blob_type == 7 ? 8 : 4),
        id:         miner.id
    };
...
function processShare(miner, job, blockTemplate, params) {
    ...
    const hashDiff = hash_buff_diff(blob_type == 8 ? multiHashing.c29_cycle_hash(params.pow) : new Buffer(params.result, 'hex'));
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (blob_type == 8) {
            const shareBuffer = cnUtil.construct_block_blob(template, bignum(nonce, 10).toBuffer({endian: 'little', size: 4}), blob_type, params.pow)
            const header = Buffer.concat([cnUtil.convert_blob(shareBuffer, blob_type), bignum(nonce, 10).toBuffer({endian: 'big', size: 4})]);
            ...
            if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
                ...
                const shareBuffer = cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), blob_type);
                const convertedBlob = cnUtil.convert_blob(shareBuffer, blob_type);
