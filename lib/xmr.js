...
function parse_algo_variant(algo_str, variant) {
    ...
    switch (algo_str) {
        ...
        case 'cryptonight/1':
        case 'cn-lite/1':
        case 'cryptonight-lite/1':    return 1;

        case 'cn-lite/ipbc':
        case 'cryptonight-lite/ipbc': return 2;

        case 'cn-lite/xtl':
        case 'cryptonight-lite/xtl':  return 3;
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (!(poolName in poolShareTime) || !(poolName in poolShareCount)) {
            ...
            console.log(`Submitted ${poolShareCount[poolName]} share(s) of ${poolShareSize[poolName]} hashes to ${poolName} pool`);
            poolShareTime[poolName] = Date.now();
