...
function detectAlgo(pool_algo, block_version) {
    if ("cn/2" in pool_algo && "cn/1" in pool_algo) return block_version >= 8 ? "cn/2" : "cn/1"; // monero fork
    if (pool_algo.length == 1) return pool_algo[0];
    console.error("Can't not correctly detect block template algorithm from the list of provided default algorithms (please reduce it to single item): " +  pool_algo.join(", "));
    return pool_algo[0];
}
