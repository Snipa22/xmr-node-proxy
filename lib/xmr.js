...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        hash = multiHashing.cryptonight(convertedBlob, convertedBlob[0] >= 8 ? 8 : 1);
        if (hash.toString("hex") !== resultHash) {
            let secondaryHash = multiHashing.cryptonight(convertedBlob, 0);
            let tertiaryHash = multiHashing.cryptonight(convertedBlob, 1);
            if (secondaryHash.toString("hex") === resultHash) {
                console.error(`${global.threadName} Invalid version of CN hashing algo (CN/0) used by miner ${miner.logString}`);
            }
            else if (tertiaryHash.toString("hex") === resultHash) {
